<?php
namespace TheCodeBunny\eBayApi\Types;

use TheCodeBunny\eBayApi\Exceptions;
use TheCodeBunny\eBayApi\JmesPath\Env;
use TheCodeBunny\eBayApi\JmesPath\JmesPathableObjectInterface;
use TheCodeBunny\eBayApi\Types;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \TheCodeBunny\eBayApi\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \TheCodeBunny\eBayApi\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \TheCodeBunny\eBayApi\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'TheCodeBunny\eBayApi\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \TheCodeBunny\eBayApi\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\TheCodeBunny\eBayApi\Types\BaseType', false)) {
