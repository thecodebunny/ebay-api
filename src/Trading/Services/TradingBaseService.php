...
use TheCodeBunny\eBayApi\Trading\Types;

/**
 * Base class for the Trading service.
 */
class TradingBaseService extends \TheCodeBunny\eBayApi\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \TheCodeBunny\eBayApi\Trading\Services\TradingService::API_VERSION,
                'required' => true
            ],
            'authorization' => [
                'valid' => ['string']
    ...
    protected function callOperationAsync($name, \TheCodeBunny\eBayApi\Types\BaseType $request, $responseClass)
    {
        ...
        if ($this->getConfig('authorization') !== null) {
             ...
             */
            if (!isset($request->RequesterCredentials)) {
                $request->RequesterCredentials = new \TheCodeBunny\eBayApi\Trading\Types\CustomSecurityHeaderType();
            }
