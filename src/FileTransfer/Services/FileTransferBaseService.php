 ...
 */
class FileTransferBaseService extends \TheCodeBunny\eBayApi\Services\BaseService
{
    /**
    ...
    public static function getConfigDefinitions()
    {
        ...
        return $definitions + [
            'apiVersion' => [
                'valid' => ['string'],
                'default' => \TheCodeBunny\eBayApi\FileTransfer\Services\FileTransferService::API_VERSION
            ],
            'authToken' => [
                'valid' => ['string'],
                'required' => true
    ...
    protected function callOperationAsync($name, \TheCodeBunny\eBayApi\Types\BaseType $request, $responseClass)
    {
        ...
        if ($name === 'uploadFile' && $request->hasAttachment()) {
            ...
            if (!isset($request->fileAttachment)) {
                $request->fileAttachment = new \TheCodeBunny\eBayApi\FileTransfer\Types\FileAttachment();
            }

            if (!isset($request->fileAttachment->Data)) {
                $request->fileAttachment->Data = new \TheCodeBunny\eBayApi\FileTransfer\Types\Data([
                    'xopInclude' => new \TheCodeBunny\eBayApi\FileTransfer\Types\XopInclude([
